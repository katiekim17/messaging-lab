...
> 총 **56개 학습 테스트**. 각 테스트 이름이 곧 증명 명제입니다.
...
| 2 | **Transactional Event** | 트랜잭션과 이벤트 타이밍은 왜 중요한가? | 7개 | Spring Event |
| 3 | **Event Store** | 서버가 죽으면 이벤트는 어디로 가는가? | 7개 | H2 (JPA) |
| 4 | **Redis Pub/Sub** | 프로세스 밖으로 이벤트를 보내면? | 4개 | Redis |
| 5 | **Kafka** | 메시지를 보존하면서 전달하려면? | 12개 | Kafka |
| 6 | **Idempotent Consumer** | 같은 메시지가 2번 오면 어떻게 되는가? | 11개 | Kafka |
...
### Step 2 — Transactional Event (7개)

| 테스트 | 증명하는 것 |
|--------|-----------|
| EventListener는 커밋 전에 실행되어 롤백시 부수효과가 되돌려지지 않는다 | 타이밍 위험 |
...
### Step 6 — Idempotent Consumer (11개)
